package com.ftd.keal.keal_mvvm.ui.adapters;
...
import com.ftd.keal.keal_mvvm.ui.download.TasksManagerModel;
...
/**
 * Created by sdhuang on 16/8/15 14:21.
 */
public class TaskItemAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder instanceof TaskItemViewHolder) {
            ...
            taskItemViewHolder.update(model.getId(), position);
            taskItemViewHolder.taskActionBtn.setTag(holder);
            taskItemViewHolder.taskNameTv.setText(model.getName());

            TasksManager.getImpl()
                    .updateViewHolder(taskItemViewHolder.id, taskItemViewHolder);
            ...
            if (TasksManager.getImpl().isReady()) {
                ...
                if (status == FileDownloadStatus.pending || status == FileDownloadStatus.started ||
                    ...
                    taskItemViewHolder.updateDownloading(status, TasksManager.getImpl().getSoFar(model.getId())
                    ...
                    // not exist file
                    taskItemViewHolder.updateNotDownloaded(status, 0, 0);
                    ...
                    // not start
                    taskItemViewHolder.updateNotDownloaded(status, TasksManager.getImpl().getSoFar(model.getId())
                            , TasksManager.getImpl().getTotal(model.getId()));
                }
            } else {
        ...
        public void updateDownloading(final int status, final long sofar, final long total) {
            ...
            switch (status) {
                ...
                case FileDownloadStatus.progress:
                    ...
                    break;
                default:
                    taskStatusTv.setText(KealMvvmApplication.CONTEXT.getString(
